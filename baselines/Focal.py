    ...
    def train(self, save_dir, num_epochs, batch_size, learning_rate, device, actions_dict,
        ...
        for epoch in range(resume_epoch, num_epochs):
            ...
            for _, items in enumerate(train_loader):
                ...
                loss = 0
                target = batch_target.view(-1) * (batch_target.view(-1) != -100).long()
                for i, p in enumerate(ps):
                    cross_entropy = self.ce_split(p.transpose(2, 1).contiguous().view(-1, self.num_classes), batch_target.view(-1))
                    input_prob = torch.gather(F.softmax(p, 1).transpose(2, 1).contiguous().view(-1, self.num_classes), 1, target.unsqueeze(1))
                    input_prob = Variable(input_prob.view(-1))
                    if i == len(ps) - 1:
                        s_ce_loss = torch.mean(torch.pow(1 - input_prob, args.gamma) * cross_entropy)
                    else:
                        s_ce_loss = torch.mean(cross_entropy)
...
TYPE = '/mstcn_focal_gamma{}_{}'.format(args.gamma, args.seed)
...
log.logger.info("Training for MS-TCN with Focal")
