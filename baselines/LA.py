...
class Trainer:
    def __init__(self, model, log, sample_rate, **kwargs):
        ...
        self.prior = torch.from_numpy(class_prior).float().to(device)
        ...
        for epoch in range(resume_epoch, num_epochs):
            ...
            for _, items in enumerate(train_loader):
                ...
                for i, p in enumerate(ps):
                    if i == len(ps) - 1:
                        logits = p.transpose(2, 1).contiguous().view(-1, self.num_classes) + torch.log(self.prior.view(1,-1)**args.tau + 1e-12)
                        s_ce_loss = self.ce(logits, batch_target.view(-1))
                    else:
...
TYPE = '/mstcn_la-tau{}_{}'.format(args.tau, args.seed)
...
log.logger.info("Training for MS-TCN with LA")
