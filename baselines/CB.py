...
class Trainer:
    def __init__(self, model, log, sample_rate, **kwargs):
        ...
        self.weights = torch.from_numpy(effective_weights).float().to(device)
        ...
        for epoch in range(resume_epoch, num_epochs):
            ...
            for _, items in enumerate(train_loader):
                ...
                for i, p in enumerate(ps):
                    if i == len(ps) - 1:
                        s_ce_loss = self.ce(weight=self.weights, ignore_index=-100) \
                                (p.transpose(2, 1).contiguous().view(-1, self.num_classes), batch_target.view(-1))
                    else:
                        s_ce_loss = self.ce(ignore_index=-100)(
                                p.transpose(2, 1).contiguous().view(-1, self.num_classes), batch_target.view(-1))
...
parser.add_argument('--beta', type=float, default=0.9, help='Beta for class balanced loss.')
...
TYPE = '/mstcn_cb_beta{}_{}'.format(args.beta, args.seed)
...
log.logger.info("Training for MS-TCN with CB")
...
### #num of frames is too large, scale to the same range as in the paper with max num =1000
effective_num = 1.0 - np.power(args.beta, frame_per_cls/int(np.max(frame_per_cls)/1000))
effective_weights = (1.0 - args.beta) / np.array(effective_num)
effective_weights = effective_weights / np.sum(effective_weights) * int(num_classes)
